...
from ...models.routing import ChannelHealth, ChannelKeyHealth, ModelHealth
...
class _HealthTracker:
    ...
    def configure(
        ...
        *,
        ...
        bucket_seconds = self._bucket_seconds(bounded_window_seconds)
        bounded_penalty_weight = min(max(health_penalty_weight, 0.0), 1.0)
        bounded_min_samples = max(health_min_samples, 1)
        scoring_changed = (
            effective_enabled != self._health_scoring_enabled
            or bounded_window_seconds != self._health_window_seconds
            or bounded_penalty_weight != self._health_penalty_weight
            or bounded_min_samples != self._health_min_samples
        )
        if (
            not scoring_changed
        ...
        self._health_scoring_enabled = effective_enabled
        if bucket_seconds != self._health_bucket_seconds:
            self._health_windows.clear()
        self._health_window_seconds = bounded_window_seconds
        self._health_bucket_seconds = bucket_seconds
        self._health_penalty_weight = bounded_penalty_weight
        self._health_min_samples = bounded_min_samples
