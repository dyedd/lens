...
def canonicalize_editable_settings(items: Iterable[SettingItem]) -> list[SettingItem]:
    canonical_items: list[SettingItem] = []
    seen: set[str] = set()
    for item in items:
        if item.key in seen:
        ...
        canonical_items.append(
            SettingItem(
                key=item.key, value=canonicalize_editable_setting(item.key, item.value)
            )
        )
...
def effective_editable_setting_items(items: Iterable[SettingItem]) -> list[SettingItem]:
    ...
    for key, default in EDITABLE_SETTING_DEFAULTS.items():
        value = resolve_effective_setting(key, stored.get(key, default), default)
        effective.append(SettingItem(key=key, value=value))
    return effective


def canonicalize_editable_setting(key: str, raw_value: str) -> str:
    ...
    if key == SETTING_TIME_ZONE:
        return validate_time_zone_name(value)
...
def resolve_effective_setting(key: str, raw_value: str, default: str) -> str:
    try:
        return canonicalize_editable_setting(key, raw_value)
    except ValueError:
        return canonicalize_editable_setting(key, default)
