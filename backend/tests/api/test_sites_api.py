...
from app.models.sites import SiteUpdate


def _rate_site_payload(
    source: str,
    *,
    group: str = "",
    ...
    payload["base_urls"][0]["url"] = base_url
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "sub2api",
            "https://upstream.example/v1?tenant=one#billing",
            "/v1/sub2api/billing",
            id="sub2api-query-fragment",
        ),
        pytest.param(
            "sub2api",
            "https://upstream.example/v1/sub2api/billing",
            "/v1/sub2api/billing",
            id="sub2api-existing-endpoint",
        ),
        pytest.param(
            "sub2api",
            "https://upstream.example/api/pricing",
            "/v1/sub2api/billing",
            id="sub2api-replaces-newapi-endpoint",
        ),
        pytest.param(
            "newapi", "https://upstream.example", "/api/pricing", id="newapi-host"
        ),
        pytest.param(
            "newapi",
            "https://upstream.example/v1/",
        ...
        pytest.param(
            ...
            "https://upstream.example/v1/sub2api/billing",
            "/api/pricing",
            id="newapi-replaces-sub2api-endpoint",
        ),
        pytest.param(
            "newapi",
            "https://upstream.example/api/pricing",
            "/api/pricing",
            id="newapi-existing-endpoint",
        ),
    ],
)
def test_credential_rate_sync_resolves_base_url_variants(
    client,
    app_state,
    admin_headers,
    create_site,
    ...
    site = create_site(
        _rate_site_payload(
            source,
            group="default" if source == "newapi" else "",
            base_url=base_url,
        )
    )
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        if source == "sub2api":
            ...
            payload = {"success": True, "group_ratio": {"default": 1}}
