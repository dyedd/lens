...
function extractModelFamilyName(name: string) {
  return (name.includes("/") ? name.split("/")[1] : name).trim().toLowerCase();
}

function getRawModelPrefix(value: string) {
  const lowerValue = value.trim().toLowerCase();
  const match = /^[a-z0-9]+/.exec(lowerValue);
  return match?.[0] || lowerValue;
}

function getModelFamilyDefinition(name: string) {
  const lowerValue = extractModelFamilyName(name);
  for (const item of ICON_ENTRIES) {
    if (item.prefixes.some((prefix) => lowerValue.startsWith(prefix))) {
      return item;
    }
  }
  return null;
