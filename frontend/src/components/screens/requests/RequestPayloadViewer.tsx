...
export function toLfLineEndings(value: string) {
...
function parseSseJsonPayloads(content: string) {
  const payloads = toLfLineEndings(content)
    .split("\n")
...
export function LineNumberedCode({ text }: { text: string }) {
  const lines = useMemo(() => toLfLineEndings(text).split("\n"), [text]);
...
/** Render a request payload as structured JSON or line-numbered text. */
export function RequestPayloadViewer({
  title,
  content,
  emptyText,
  locale,
