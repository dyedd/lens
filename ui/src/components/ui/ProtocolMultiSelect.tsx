...
export interface ProtocolMultiSelectProps {
  ...
  disabled?: boolean;
  invalid?: boolean;
  shouldRequireAtLeastOne?: boolean;
  placeholder?: string;
}
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div
        className={cn(
          "grid gap-0.5",
          isMultiColumn ? "grid-cols-2" : "grid-cols-[minmax(0,max-content)]",
        )}
      >
...
/** Render a constrained multi-select for supported protocols. */
export function ProtocolMultiSelect({
  ...
  const isMultiColumn = clientProtocols.length > 4;
  ...
  const clearDisabled = shouldRequireAtLeastOne && value.length <= 1;
