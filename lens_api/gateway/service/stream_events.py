...
def _capture_stream_event_chunk(
    ...
    if terminal_boundary is not None:
        normalized_prefix_length = skipped_prefix_length + max(
            terminal_boundary - pending_length, 0
        )
        return _raw_prefix_length_for_normalized_text(
            raw_text,
...
def _raw_prefix_length_for_normalized_text(
    raw_text: str,
    normalized_prefix_length: int,
    *,
    ...
    while source_index < len(source):
        if source[source_index] == "\r" and source_index + 1 < len(source):
            ...
            source_index += 1
        normalized_length += 1
        if normalized_length >= normalized_prefix_length:
