...
from ...core.url_utils import append_url_path
from ...models import ChannelConfig, ChannelStatus, SiteConfig, SiteCredential
...
def _sub2api_billing_endpoint(base_url: str) -> str:
    ...
    if parts and re.fullmatch(r"v\d+", parts[-1]):
        return append_url_path(base_url, "sub2api", "billing")
    return append_url_path(base_url, "v1", "sub2api", "billing")


def _newapi_pricing_endpoint(base_url: str) -> str:
    url = httpx.URL(base_url.strip())
    path = url.path.rstrip("/")
    ...
    if parts and re.fullmatch(r"v\d+", parts[-1]):
        parts.pop()
    root = str(url.copy_with(path="/" + "/".join(parts)))
    return append_url_path(root, "api", "pricing")
...
async def _fetch_credential_rate(
    ...
    if credential.rate_source == "sub2api":
        ...
        endpoint = _sub2api_billing_endpoint(str(channel.base_url))
        default_headers = {
        ...
        endpoint = _newapi_pricing_endpoint(str(channel.base_url))
