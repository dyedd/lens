...
- 直接调用：渠道里的可用模型按原名即可调用，多个渠道同名时自动故障切换
- 模型组路由：按“运行时渠道 + 凭证 + 上游模型”组成候选，支持实时自动包含规则、轮询、故障切换、执行模型组复用、渠道级启停和多模态回退
...
渠道模型已可按原名直接调用，模型组只在需要时创建：给模型起别名、把多个上游模型名合并成一个、或自定义候选顺序和策略。同名时模型组优先于渠道模型。

进入 `/groups`，新建模型组，添加上游模型，选择路由策略。在候选列表搜索后点击“设为自动包含规则”，所有渠道中名称匹配的可用模型会实时成为成员，之后新增的渠道或同步来的模型也会自动加入；关闭或移除规则成员即可将其排除。可服务的客户端协议由成员决定：Auto 成员可接全部入口；指定了上游协议的成员，只接该协议以及能转换到它的客户端协议。
...
**协议转换**：成员指定了上游协议、且与客户端协议不同时才转换。目前支持 Anthropic → OpenAI Chat / Responses，OpenAI Responses → OpenAI Chat，OpenAI Chat → OpenAI Responses。
...
客户端只需要：Lens Base URL + 网关 API Key + 模型名（模型组名称或渠道中的上游模型名）。`/v1/models` 会同时列出两者。
