...
- 站点管理：一个站点可配置多个 Base URL 和多个凭证，支持模型同步、手动模型、批量导入和站点标签
...
进入 `/channels`，新建站点，配置 Base URL 和凭证，然后同步或手动添加模型。请求头、代理和参数覆盖在站点上配置。
...
进入 `/groups`，新建模型组，添加上游模型，选择路由策略。可服务的客户端协议由成员决定：Auto 成员可接全部入口；指定了上游协议的成员，只接该协议以及能转换到它的客户端协议。
